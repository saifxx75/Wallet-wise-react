...
const Sidebar = () => {
  ...
  // Update Percentage Spent whenever budget or expenses change
  useEffect(() => {
    if (totalBudget > 0) {
      setPercentageSpent(((amountSpent / totalBudget) * 100).toFixed(2));
    } else {
  ...
  const handleAddBudget = () => {
    const budget = prompt("Enter Total Budget:");
  ...
  const handleAddExpense = () => {
    const expense = prompt("Enter Amount Spent:");
    if (expense && !isNaN(expense) && expense > 0) {
      setAmountSpent((prev) => prev + parseFloat(expense));
    } else {
      alert("Please enter a valid number!");
    }
  };
